...
<template>
  <div class="add">
    <!-- 试卷信息显示 -->
    <div class="exam-info-banner" v-if="description">
      <el-alert :title="'当前试卷：' + subject" :description="description" type="info" show-icon :closable="false"></el-alert>
    </div>
        ...
        <section class="append">
          <ul>
            ...
            <li v-if="optionValue == '主观题'">
              <span>所属章节：</span>
              <el-input placeholder="请输入对应章节" v-model="postEssay.section" class="w150" clearable>
              </el-input>
            </li>
            <li v-if="optionValue == '选择题'">
              <span>难度等级:</span>
            ...
            <li v-if="optionValue == '主观题'">
              <span>题目分值:</span>
              <el-input-number v-model="postEssay.score" :min="1" :max="20" class="w150"></el-input-number>
            </li>
            <li v-if="optionValue == '选择题'">
              <span>正确选项:</span>
          ...
          <!-- 主观题 -->
          <div class="change essay" v-if="optionValue == '主观题'">
            <div class="title">
              <el-tag>题目:</el-tag><span>在下面的输入框中输入主观题题目</span>
              <el-input type="textarea" rows="4" v-model="postEssay.question" placeholder="请输入题目内容" resize="none"
                class="answer">
              </el-input>
            </div>
            <div class="title">
              <el-tag type="success">参考答案:</el-tag><span>在下面的输入框中输入参考答案</span>
              <el-input type="textarea" rows="6" v-model="postEssay.referenceAnswer" placeholder="请输入参考答案" resize="none"
                class="answer">
              </el-input>
            </div>
            <div class="title">
              <el-tag type="warning">评分标准:</el-tag><span>在下面的输入框中输入评分标准/要点</span>
              <el-input type="textarea" rows="4" v-model="postEssay.scoringCriteria" placeholder="请输入评分标准，如：关键点1(2分) 关键点2(3分)等" resize="none"
                class="answer">
              </el-input>
            </div>
            <div class="submit">
...
export default {
  ...
  methods: {
    ...
    getParams() {
      ...
      let description = this.$route.query.description; //获取试卷介绍
    ...
    essaySubmit() {
      ...
      this.$axios({
        ...
        .then(() => {
          this.$axios(`/api/essay/lastId`).then((res) => {
            //获取当前题目的questionId
            let questionId = res.data.data.questionId;
            this.postPaper.questionId = questionId;
...
<style>
/* 深色模式专用样式 */
.dark-theme .add .append .change {
  background-color: #1e2d3a !important;
  border: 1px solid #3a4a5a !important;
}
.dark-theme .add .append .change .title {
  color: #b0c4d8 !important;
}

/* 深色模式 - 试卷介绍横幅 */
.dark-theme .add .exam-info-banner .el-alert--info {
  background-color: #1e3a5a !important;
  border: 1px solid #3a6a8a !important;
}
.dark-theme .add .exam-info-banner .el-alert__title {
  color: #7dd3fc !important;
}
.dark-theme .add .exam-info-banner .el-alert__description {
  color: #e0f0ff !important;
}
.dark-theme .add .exam-info-banner .el-alert__icon {
  color: #7dd3fc !important;
}

/* 深色模式 - 题目标签和提示文字区域 */
.dark-theme .add .append .change .title,
.dark-theme .add .append .fill .title,
.dark-theme .add .append .judge .title,
.dark-theme .add .append .essay .title {
  background-color: #1e3a5a !important;
  border: 1px solid #3a6a8a !important;
  border-radius: 4px;
...
.dark-theme .add .append .change .title span,
.dark-theme .add .append .fill .title span,
.dark-theme .add .append .judge .title span,
.dark-theme .add .append .essay .title span {
  color: #e0f0ff !important;
}
.dark-theme .add .append .el-tag {
  background-color: #1e3a5a !important;
  border-color: #3a6a8a !important;
  color: #7dd3fc !important;
}
.dark-theme .add .append .el-tag--success {
  background-color: #1e3a5a !important;
  border-color: #3a6a8a !important;
  color: #7dd3fc !important;
}

/* 深色模式 - 题目/解析提示区域内的文字改为黑色 */
.dark-theme .add .append .change .title span,
.dark-theme .add .append .fill .title span,
.dark-theme .add .append .judge .title span,
.dark-theme .add .append .essay .title span {
