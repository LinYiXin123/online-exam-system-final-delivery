  ...
  <div id="myExam">
    ...
    <div class="wrapper">
      <ul class="top">
        ...
        <li><el-button type="primary" @click="search()" class="search-btn"> 搜索试卷</el-button></li>
      </ul>
      <!-- 卡片视图 -->
        ...
        <el-table :data="pagination.records" style="width: 100%" @row-click="toExamMsg" :row-style="{cursor: 'pointer'}">
          <el-table-column label="科目名称" min-width="150">
            <template slot-scope="scope">
              <i class="iconfont icon-r-edit" style="font-size: 18px; margin-right: 8px; color: var(--primary-color, #0195ff);"></i>
              <span style="font-weight: 600;">{{ scope.row.source }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="description" label="练习描述" min-width="200"></el-table-column>
          <el-table-column label="练习时间" width="130">
            <template slot-scope="scope">
          ...
          <el-table-column label="满分" width="80">
            <template slot-scope="scope">
              <el-tag type="success" size="small">{{ scope.row.totalScore }}分</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="100">
            <template slot-scope="scope">
              <el-button type="success" size="mini" @click.stop="toExamMsg(scope.row)">练习</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
.table-view .el-table tr:hover > td {
  background-color: var(--primary-light, #e6ffed) !important;
}
</style>

<style>
/* 全局样式 - 确保视图切换按钮使用主题色 */
#startExam .view-switch .el-button.active {
  background-color: var(--primary-color, #0195ff) !important;
  border-color: var(--primary-color, #0195ff) !important;
  color: #fff !important;
}

#startExam .view-switch .el-button.active i {
  color: #fff !important;
}

/* 搜索按钮美化样式 */
#startExam .search-btn {
  background-color: var(--primary-color, #409EFF) !important;
  border-color: var(--primary-color, #409EFF) !important;
  color: #fff !important;
}

#startExam .search-btn span {
  color: #fff !important;
}

#startExam .search-btn i {
  color: #fff !important;
}

#startExam .search-btn:hover {
  opacity: 0.9;
}
</style>
